...
import org.apache.spark.sql.catalyst.expressions.{Ascending, Attribute, Expression, JoinedRow, NamedExpression, OffsetWindowFunction, SortOrder, SpecificInternalRow, UnsafeProjection, UnsafeRow}
import org.apache.spark.sql.catalyst.plans.physical.{AllTuples, ClusteredDistribution, Distribution, Partitioning}
import org.apache.spark.sql.execution.direct.DirectPlan
import org.apache.spark.sql.execution.{ExternalAppendOnlyUnsafeRowArray, SortExec}
import org.apache.spark.sql.execution.window.{RangeBoundOrdering, RowBoundOrdering, WindowExecBase, WindowFunctionFrame}
...
/**
  ...
  */
case class WindowDirectExec(windowExpression: Seq[NamedExpression],
  ...
  override def doExecute(): Iterator[InternalRow] = {
    ...
    new Iterator[InternalRow] {
      ...
      private[this] def fetchNextRow() {
        nextRowAvailable = iterator.hasNext
        if (nextRowAvailable) {
          nextRow = iterator.next().asInstanceOf[UnsafeRow]
