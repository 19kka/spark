...
import org.apache.spark.sql.execution.direct.window.AggregateDirectProcessor
...
abstract class WindowExecBase(windowExpression: Seq[NamedExpression],
  ...
  /**
    ...
    */
  protected def createResultProjection(
  ...
  /**
    ...
    */
  private def createBoundOrdering(frame: FrameType,
                                  bound: Expression,
    ...
    (frame, bound) match {
      ...
      case (RangeFrame, offset: Expression) if orderSpec.size == 1 =>
        ...
        val boundSortExprs = sortExpr.copy(
          BoundReference(0, expr.dataType, expr.nullable)
        ) :: Nil
    ...
    windowExpression.foreach { x =>
      x.foreach {
        case e @ WindowExpression(function, spec) =>
          ...
          function match {
            case AggregateExpression(f, _, _, _) =>
